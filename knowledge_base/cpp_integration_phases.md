...
- `GhostRigger.Renderer.D3D12.dll` exists as the first hardware renderer backend
  ...
  metadata, report guarded draw-command recording metadata, and report
...
- `GhostRigger.Renderer.D3D12.DEBUG.exe` validates the D3D12 renderer package
  ...
  recording metadata export, failure-diagnostic export, and
  device-requirement metadata
  without Python or the GUI.
...
4. Extend `GhostRigger.Runtime.Shared.Resources` when future renderer-neutral
   ...
   schema metadata.
5. Extend `GhostRigger.Renderer.D3D12` from guarded draw-command recording
   metadata into guarded draw-submission readiness metadata before enabling
