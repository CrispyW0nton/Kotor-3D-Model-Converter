...
- `native/GhostRigger.Renderer.D3D12/` owns the first hardware renderer backend
  package boundary behind the renderer contract boundary. It is diagnostic-only
  in Phase 1 and does not create a device or submit draws yet.
- `native/GhostRigger.Renderer.D3D12.DEBUG/` verifies the D3D12 renderer package
  ABI and device-requirement metadata from Visual Studio without requiring
  Python.
