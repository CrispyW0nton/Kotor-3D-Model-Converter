...
- `native/GhostRigger.Renderer.D3D12/` owns the first hardware renderer backend
  ...
  Phase 1 and does not create swap chains, execute command lists, or submit
  draws yet.
- `native/GhostRigger.Renderer.D3D12.DEBUG/` verifies the D3D12 renderer package
  ...
  export, failure-diagnostic export, and device-requirement metadata from Visual
  Studio without requiring Python.
