#include "Tools_SequenceEditor/SequenceContracts.h"
...
double ease(double t, const char* mode) noexcept {
    ...
    const std::string interpolation = normalize_interpolation_mode(mode);
    ...
    if (interpolation == "Ease In Out" || interpolation == "Cubic") {
...
double interpolate_number(double a, double b, double t, const char* mode) noexcept {
    ...
    return lerp_number(a, b, ease(t, interpolation.c_str()));
}
...
int interpolate_bool(int a, int b, double t, const char* mode) noexcept {
    ...
    return ease(t, interpolation.c_str()) < 1.0 ? (a ? 1 : 0) : (b ? 1 : 0);
}
...
const char* sequence_contracts_schema_json() noexcept {
    return R"({"schema":"sequence_contracts_native.v1",)"
           ...
           R"("python_fallback":["sequence editor UI","mapping/list recursive interpolation","keyframe object sorting/evaluation","track mutation","sequence serialization","asset file IO","viewport/object evaluator application","render output"],)"
...
__declspec(dllexport) const char* gr_sequence_interpolation_mode(const char* mode) {
    return ghostrigger::core::tools::sequenceeditor::contracts::normalize_interpolation_mode(mode);
}

__declspec(dllexport) double gr_sequence_ease(double t, const char* mode) {
    return ghostrigger::core::tools::sequenceeditor::contracts::ease(t, mode);
}

__declspec(dllexport) double gr_sequence_lerp_number(double a, double b, double t) {
    return ghostrigger::core::tools::sequenceeditor::contracts::lerp_number(a, b, t);
}

__declspec(dllexport) double gr_sequence_interpolate_number(double a, double b, double t, const char* mode) {
    return ghostrigger::core::tools::sequenceeditor::contracts::interpolate_number(a, b, t, mode);
}

__declspec(dllexport) int gr_sequence_interpolate_bool(int a, int b, double t, const char* mode) {
    return ghostrigger::core::tools::sequenceeditor::contracts::interpolate_bool(a, b, t, mode);
}

__declspec(dllexport) int gr_sequence_clamp_frame(int frame, int start_frame, int end_frame) {
    return ghostrigger::core::tools::sequenceeditor::contracts::clamp_frame(frame, start_frame, end_frame);
}

__declspec(dllexport) double gr_sequence_frame_to_seconds(int frame, int start_frame, double frame_rate) {
    return ghostrigger::core::tools::sequenceeditor::contracts::frame_to_seconds(frame, start_frame, frame_rate);
}

__declspec(dllexport) int gr_sequence_seconds_to_frame(double seconds, int start_frame, int end_frame, double frame_rate) {
    return ghostrigger::core::tools::sequenceeditor::contracts::seconds_to_frame(
        seconds,
        start_frame,
        end_frame,
        frame_rate
...
__declspec(dllexport) int gr_sequence_duration_frames(int start_frame, int end_frame) {
    return ghostrigger::core::tools::sequenceeditor::contracts::duration_frames(start_frame, end_frame);
}

__declspec(dllexport) double gr_sequence_duration_seconds(int start_frame, int end_frame, double frame_rate) {
    return ghostrigger::core::tools::sequenceeditor::contracts::duration_seconds(start_frame, end_frame, frame_rate);
}

__declspec(dllexport) const char* gr_sequence_contracts_schema_json() {
    return ghostrigger::core::tools::sequenceeditor::contracts::sequence_contracts_schema_json();
}
