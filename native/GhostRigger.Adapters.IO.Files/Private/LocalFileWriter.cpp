...
namespace ghostrigger::adapters::io::files::local_file_writer {
namespace {
...
GHOSTRIGGER_ADAPTERS_FILES_API int gr_adapters_files_write_bytes(
    ...
    return ghostrigger::adapters::io::files::local_file_writer::write_bytes(
...
GHOSTRIGGER_ADAPTERS_FILES_API int gr_adapters_files_write_text(
    ...
    return ghostrigger::adapters::io::files::local_file_writer::write_text(
        path == nullptr ? std::string{} : std::string(path),
...
GHOSTRIGGER_ADAPTERS_FILES_API int gr_adapters_files_write_text_utf8(
    ...
    return ghostrigger::adapters::io::files::local_file_writer::write_text_utf8(
        path == nullptr ? std::string{} : std::string(path),
        text == nullptr ? std::string{} : std::string(text)
    ) ? 1 : 0;
}
