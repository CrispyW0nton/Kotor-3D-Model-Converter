...
int main()
{
    const char* version = gr_validation_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_validation_capabilities_json();
    ...
    const char* dependency_schema = gr_validation_dependency_schema_json();
