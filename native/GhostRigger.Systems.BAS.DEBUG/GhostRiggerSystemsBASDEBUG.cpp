...
int main()
{
    const char* version = gr_systems_bas_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_systems_bas_capabilities_json();
    ...
    if (std::strstr(gr_systems_bas_owner_boundary_json(), R"("schema":"systems_bas_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_systems_bas_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Systems.BAS.DEBUG OK: " << version << std::endl;
    return 0;
}