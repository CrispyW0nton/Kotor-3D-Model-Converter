...
int main()
{
    const char* version = gr_diagnostics_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_diagnostics_capabilities_json();
    ...
    if (std::strstr(gr_diagnostics_owner_boundary_json(), R"("schema":"diagnostics_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_diagnostics_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Diagnostics.DEBUG OK: " << version << std::endl;
    return 0;
}