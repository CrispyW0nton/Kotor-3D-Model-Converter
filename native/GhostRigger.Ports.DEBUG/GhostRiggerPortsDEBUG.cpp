...
int main()
{
    const char* version = gr_ports_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_ports_capabilities_json();
    ...
    if (std::strstr(gr_ports_owner_boundary_json(), R"("schema":"ports_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_ports_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Ports.DEBUG OK: " << version << std::endl;
    return 0;
}