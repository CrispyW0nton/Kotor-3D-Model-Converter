...
int main()
{
    const char* version = gr_camera_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_camera_capabilities_json();
    ...
    if (std::strstr(gr_camera_owner_boundary_json(), R"("schema":"camera_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_camera_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Camera.DEBUG OK: " << version << std::endl;
    return 0;
}