...
int main()
{
    const char* version = gr_unreal_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_unreal_capabilities_json();
    ...
    if (std::strstr(gr_unreal_owner_boundary_json(), R"("schema":"unreal_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_unreal_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Unreal.DEBUG OK: " << version << std::endl;
    return 0;
}