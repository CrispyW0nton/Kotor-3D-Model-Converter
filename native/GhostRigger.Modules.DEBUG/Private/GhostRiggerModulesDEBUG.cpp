...
int main()
{
    const char* version = gr_modules_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_modules_capabilities_json();
    ...
    if (std::strstr(gr_modules_owner_boundary_json(), R"("schema":"modules_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_modules_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Modules.DEBUG OK: " << version << std::endl;
    return 0;
}