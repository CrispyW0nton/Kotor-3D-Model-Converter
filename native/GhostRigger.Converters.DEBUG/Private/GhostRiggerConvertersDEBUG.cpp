...
int main()
{
    const char* version = gr_converters_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_converters_capabilities_json();
    ...
    if (std::strstr(gr_converters_owner_boundary_json(), R"("schema":"converters_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_converters_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Converters.DEBUG OK: " << version << std::endl;
    return 0;
}