...
int main()
{
    const char* version = gr_formats_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_formats_capabilities_json();
    ...
    if (std::strstr(gr_formats_owner_boundary_json(), R"("schema":"formats_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_formats_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Formats.DEBUG OK: " << version << std::endl;
    return 0;
}