...
_NATIVE_GPU_ADAPTER_ENV = "GHOSTRIGGER_CORE_RENDERING_GPU"
_NATIVE_GPU_ADAPTER_DLL = "GhostRigger.Core.Rendering.GPU.dll"
...
def _native_gpu_adapter_candidates() -> Tuple[Path, ...]:
    ...
    return (
        ...
        root / "native" / "GhostRigger.Core.Rendering.GPU" / "build" / "vs" / "x64" / "Debug" / _NATIVE_GPU_ADAPTER_DLL,
        root / "native" / "GhostRigger.Core.Rendering.GPU" / "build" / "vs" / "x64" / "Release" / _NATIVE_GPU_ADAPTER_DLL,
    )
...
def _native_gl_context_backend_candidates(os_name: Optional[str] = None) -> Tuple[str, ...]:
    ...
    try:
        ...
        function = dll.gr_core_rendering_gpu_gl_backend_candidates_json
