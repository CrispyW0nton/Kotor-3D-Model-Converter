...
int main()
{
    const char* version = gr_workflow_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_workflow_capabilities_json();
    ...
    if (std::strstr(gr_workflow_owner_boundary_json(), R"("schema":"workflow_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_workflow_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Workflow.DEBUG OK: " << version << std::endl;
    return 0;
}