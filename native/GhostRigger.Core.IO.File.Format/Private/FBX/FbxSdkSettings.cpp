#include "FBX/FbxSdkSettings.h"

#include <algorithm>
#include <cctype>

namespace ghostrigger::core::io::fbx::sdk_settings {
namespace {
...
__declspec(dllexport) const char* gr_io_fbx_sdk_download_url() {
    return ghostrigger::core::io::fbx::sdk_settings::fbx_download_url();
}

__declspec(dllexport) const char* gr_io_fbx_sdk_licence_notice() {
    return ghostrigger::core::io::fbx::sdk_settings::licence_notice();
}

__declspec(dllexport) const char* gr_io_fbx_sdk_recommended_fix(const char* error) {
    thread_local std::string result;
    result = ghostrigger::core::io::fbx::sdk_settings::recommended_fix(error == nullptr ? "" : std::string(error));
    return result.c_str();
}

__declspec(dllexport) const char* gr_io_fbx_sdk_settings_contracts_schema_json() {
    return ghostrigger::core::io::fbx::sdk_settings::fbx_sdk_settings_contracts_schema_json();
}
