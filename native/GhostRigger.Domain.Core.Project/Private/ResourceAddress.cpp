...
namespace ghostrigger::domain::core::project::core::project::resource_address {
namespace {
...
const char* resource_address_contracts_schema_json() noexcept {
    static constexpr const char* kJson =
        R"({"schema":"project_resource_address_native.v1",)"
        R"("source":"src/core/project/resource_address.py",)"
        R"("native_scope":["supported scheme registry","ResourceAddress normalization","stable key formatting","display name formatting","flat JSON serialization"],)"
        R"("python_fallback":["ResourceAddress.from_dict object coercion","arbitrary metadata value preservation","project model graph serialization","project validation issue generation"],)"
        R"("reason_python_fallback":"dynamic Python object coercion and project graph validation remain Python-owned until their callers are ported subsystem-by-subsystem"})";
    return kJson;
}
...
__declspec(dllexport) const char* gr_project_resource_address_supported_schemes_json() {
    static const std::string schemes = ghostrigger::domain::core::project::core::project::resource_address::supported_schemes_json();
    return schemes.c_str();
}

__declspec(dllexport) int gr_project_resource_address_is_supported_scheme(const char* scheme) {
    return ghostrigger::domain::core::project::core::project::resource_address::is_supported_scheme(
        std::string(scheme == nullptr ? "" : scheme)
    ) ? 1 : 0;
}

__declspec(dllexport) const char* gr_project_resource_address_stable_key(
    const char* scheme,
    const char* game,
    const char* module_id,
    const char* resref,
    ...
    result = ghostrigger::domain::core::project::core::project::resource_address::make_address_from_c_args(
        scheme,
        game,
        module_id,
        resref,
...
__declspec(dllexport) const char* gr_project_resource_address_display_name(
    const char* scheme,
    const char* game,
    const char* module_id,
    const char* resref,
...
__declspec(dllexport) const char* gr_project_resource_address_to_json(
    const char* scheme,
    const char* game,
    const char* module_id,
    const char* resref,
...
__declspec(dllexport) const char* gr_project_resource_address_contracts_schema_json() {
    return ghostrigger::domain::core::project::core::project::resource_address::resource_address_contracts_schema_json();
}
