...
int main()
{
    const char* version = gr_project_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_project_capabilities_json();
    ...
    if (std::strstr(gr_project_owner_boundary_json(), R"("schema":"project_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_project_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Project.DEBUG OK: " << version << std::endl;
    return 0;
}