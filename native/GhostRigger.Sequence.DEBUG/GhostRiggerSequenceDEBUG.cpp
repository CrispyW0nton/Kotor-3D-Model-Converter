...
int main()
{
    const char* version = gr_sequence_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_sequence_capabilities_json();
    ...
    if (std::strstr(gr_sequence_owner_boundary_json(), R"("schema":"sequence_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_sequence_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Sequence.DEBUG OK: " << version << std::endl;
    return 0;
}