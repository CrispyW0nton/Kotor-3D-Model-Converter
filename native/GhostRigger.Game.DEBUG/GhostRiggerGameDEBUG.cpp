...
int main()
{
    const char* version = gr_game_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_game_capabilities_json();
    ...
    if (std::strstr(gr_game_owner_boundary_json(), R"("schema":"game_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_game_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Game.DEBUG OK: " << version << std::endl;
    return 0;
}