...
int main()
{
    const char* version = gr_export_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_export_capabilities_json();
    ...
    if (std::strstr(gr_export_owner_boundary_json(), R"("schema":"export_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_export_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Export.DEBUG OK: " << version << std::endl;
    return 0;
}