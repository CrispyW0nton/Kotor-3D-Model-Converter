...
int main()
{
    const char* version = gr_assets_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_assets_capabilities_json();
    ...
    if (std::strstr(gr_assets_owner_boundary_json(), R"("schema":"assets_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_assets_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Assets.DEBUG OK: " << version << std::endl;
    return 0;
}