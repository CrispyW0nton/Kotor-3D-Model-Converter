...
int main()
{
    const char* version = gr_walkmesh_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_walkmesh_capabilities_json();
    ...
    if (std::strstr(gr_walkmesh_owner_boundary_json(), R"("schema":"walkmesh_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_walkmesh_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Walkmesh.DEBUG OK: " << version << std::endl;
    return 0;
}