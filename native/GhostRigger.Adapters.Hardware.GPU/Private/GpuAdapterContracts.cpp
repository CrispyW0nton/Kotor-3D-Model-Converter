...
GHOSTRIGGER_ADAPTERS_GPU_API const char* gr_adapters_gpu_gl_backend_candidates_json(
    ...
    json = json_string_array(ghostrigger::adapters::hardware::gpu::gl_context_backend_candidates(os_name));
    return json.c_str();
}
...
GHOSTRIGGER_ADAPTERS_GPU_API int gr_adapters_gpu_light_kind_code(
    ...
    return ghostrigger::adapters::hardware::gpu::light_kind_code(light_kind, ambient_only != 0);
}
