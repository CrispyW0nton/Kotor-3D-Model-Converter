...
def _make_are_bytes(request: DevModuleSmokeRequest) -> bytes:
    gff = _new_gff("ARE")
    root = gff.root
    _set_loc(root, "Name", "GhostRigger Dev Test")
    root.set_string("Tag", request.module_root)
    root.set_string("Comments", "Generated by GhostRigger Map Studio T2601 smoke builder.")
    root.set_uint8("Unescapable", 0)
    root.set_uint8("DisableTransit", 0)
    root.set_uint8("Natural", 1)
    root.set_uint8("PlayerVsPlayer", 0)
    root.set_uint8("NoRest", 0)
    root.set_uint32("SunAmbientColor", 0x404040)
    root.set_uint32("SunDiffuseColor", 0xFFFFFF)
    root.set_single("FogNear", 100.0)
    root.set_single("FogFar", 200.0)
    root.set_uint32("FogColor", 0)
    root.set_uint8("SunFogOn", 0)
    return _bytes_gff(gff)


def _make_ifo_bytes(request: DevModuleSmokeRequest) -> bytes:
    gff = _new_gff("IFO")
    root = gff.root
    _set_loc(root, "Mod_Name", "GhostRigger Dev Test")
    root.set_string("Mod_Tag", request.module_root)
    apply_entry_point_to_ifo(root, _make_gameplay_placement(request).entry_point)
    root.set_uint8("Mod_DawnHour", 6)
    root.set_uint8("Mod_DuskHour", 18)
    root.set_uint8("Mod_MinPerHour", 2)
    root.set_uint32("Expansion_Pack", 0)
    return _bytes_gff(gff)
...
def build_dev_test_module(request: DevModuleSmokeRequest | None = None) -> AuthoredDevModule:
    ...
    packaged = [
        _make_packaged(root, "are", _make_are_bytes(request), "map_studio:t2601:are"),
        _make_packaged(root, "git", _make_git_bytes(request), "map_studio:t2601:git"),
        _make_packaged(root, "ifo", _make_ifo_bytes(request), "map_studio:t2601:ifo"),
