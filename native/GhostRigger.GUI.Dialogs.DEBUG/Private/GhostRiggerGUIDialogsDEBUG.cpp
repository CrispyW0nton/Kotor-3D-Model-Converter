...
int main()
{
    const char* version = gr_gui_dialogs_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_gui_dialogs_capabilities_json();
    ...
    if (std::strstr(gr_gui_dialogs_owner_boundary_json(), R"("schema":"gui_dialogs_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_gui_dialogs_dependency_schema_json();
    ...
    std::cout << "GhostRigger.GUI.Dialogs.DEBUG OK: " << version << std::endl;
    return 0;
}