"""GModeler marking menu for Map Studio — true ZModeler-style sectioned panel.

Presentation-only widget: renders a `MapStudioMarkingMenuTree` as a single
cursor-anchored panel that mirrors the real ZBrush ZModeler popup (verified
frame-by-frame from capture video): a header strip ("GMODELER | FACE
ACTIONS"), an ACTIONS grid in up to three columns, a TARGET grid for the
current action, and a MODIFIERS hint strip — all visible at once, with the
current action and target highlighted.  The action vocabulary, targets,
modifiers, and KOTOR guardrails are owned by
``src.core.modules.map_studio_marking_menu_registry``; this widget must not
define or mutate workflow policy.
...
- clicking a TARGET re-targets the current action and keeps the panel open
  (configure-then-act, like ZModeler); the choice is sticky per action
- the current action per hover context is sticky across reopens (ZModeler
  keeps QMesh current until you pick another action)
...
class MapStudioGModelerMarkingMenu(QtWidgets.QWidget):
    """Cursor-anchored ZModeler-style sectioned panel built from a registry tree."""
    ...
    def __init__(self, tree, hover_context=None, parent: QtWidgets.QWidget | None = None) -> None:
        ...
        self.setObjectName("mapStudioGModelerMarkingMenu")
