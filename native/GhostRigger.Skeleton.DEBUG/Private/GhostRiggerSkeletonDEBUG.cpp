...
int main()
{
    const char* version = gr_skeleton_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_skeleton_capabilities_json();
    ...
    if (std::strstr(gr_skeleton_owner_boundary_json(), R"("schema":"skeleton_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_skeleton_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Skeleton.DEBUG OK: " << version << std::endl;
    return 0;
}