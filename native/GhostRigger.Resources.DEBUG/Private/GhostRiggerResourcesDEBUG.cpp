...
int main()
{
    const char* version = gr_resources_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_resources_capabilities_json();
    ...
    if (std::strstr(gr_resources_owner_boundary_json(), R"("schema":"resources_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_resources_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Resources.DEBUG OK: " << version << std::endl;
    return 0;
}