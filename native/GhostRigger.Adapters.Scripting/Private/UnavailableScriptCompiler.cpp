...
std::string reason_or_default(const char* reason) {
    ...
    return text.empty() ? ghostrigger::adapters::scripting::default_unavailable_reason() : text;
}
...
GHOSTRIGGER_ADAPTERS_SCRIPTS_API const char* gr_adapters_scripts_unavailable_default_reason() {
    return ghostrigger::adapters::scripting::default_unavailable_reason();
}
...
GHOSTRIGGER_ADAPTERS_SCRIPTS_API const char* gr_adapters_scripts_unavailable_issue_json(
    ...
    issue = ghostrigger::adapters::scripting::unavailable_validation_issue_json(source, game, reason);
    return issue.c_str();
}
...
GHOSTRIGGER_ADAPTERS_SCRIPTS_API const char* gr_adapters_scripts_unavailable_compile_result_json(
    ...
    result = ghostrigger::adapters::scripting::unavailable_compile_result_json(source, game, reason);
    return result.c_str();
}
