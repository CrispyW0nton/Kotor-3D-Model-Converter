...
int main()
{
    const char* version = gr_ipc_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_ipc_capabilities_json();
    ...
    if (std::strstr(gr_ipc_owner_boundary_json(), R"("schema":"ipc_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_ipc_dependency_schema_json();
    ...
    std::cout << "GhostRigger.IPC.DEBUG OK: " << version << std::endl;
    return 0;
}