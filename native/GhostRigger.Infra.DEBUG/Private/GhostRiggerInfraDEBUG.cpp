...
int main()
{
    const char* version = gr_infra_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_infra_capabilities_json();
    ...
    if (std::strstr(gr_infra_owner_boundary_json(), R"("schema":"infra_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_infra_dependency_schema_json();
    ...
    std::cout << "GhostRigger.Infra.DEBUG OK: " << version << std::endl;
    return 0;
}