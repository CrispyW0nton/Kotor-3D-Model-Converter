...
int main()
{
    const char* version = gr_mdl_version();
    if (std::strcmp(version, "0.1.0") != 0) {
        return 1;
    }
    const char* capabilities = gr_mdl_capabilities_json();
    ...
    if (std::strstr(gr_mdl_owner_boundary_json(), R"("schema":"mdl_owner_boundary.v1")") == nullptr) {
        return 4;
    }
    const char* dependency_schema = gr_mdl_dependency_schema_json();
    ...
    std::cout << "GhostRigger.MDL.DEBUG OK: " << version << std::endl;
    return 0;
}