"""Material and map override tracks."""
...
@register_track
@dataclass
class MaterialTrack(SequenceTrack):
    TRACK_TYPE = "Material"
    property_name: str = "opacity"

    def __post_init__(self) -> None:
        self.track_type = self.TRACK_TYPE
        self.property_name = str(self.property_name or self.metadata.get("property_name") or "opacity")
    ...
    def default_interpolation(self) -> InterpolationMode:
        if self.property_name.endswith("_enabled"):
    ...
    @classmethod
    def deserialize(cls, data: dict[str, Any] | None) -> "MaterialTrack":
        payload = dict(data or {})
        return cls(
            track_id=str(payload.get("track_id") or ""),
            name=str(payload.get("name") or payload.get("property_name") or "Material"),
            track_type=cls.TRACK_TYPE,
            enabled=bool(payload.get("enabled", True)),
            muted=bool(payload.get("muted", False)),
            locked=bool(payload.get("locked", False)),
            expanded=bool(payload.get("expanded", True)),
            color=str(payload.get("color") or "#C18CFF"),
            keyframes=payload.get("keyframes", []) or [],
            parent_binding_id=str(payload.get("parent_binding_id") or ""),
            metadata=dict(payload.get("metadata", {}) or {}),
            property_name=str(payload.get("property_name") or dict(payload.get("metadata", {}) or {}).get("property_name") or "opacity"),
        )
