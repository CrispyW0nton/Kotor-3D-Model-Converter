...
def _load_dll() -> ctypes.CDLL:
    ...
    dll.gr_adapters_scripts_capabilities_json.argtypes = []
    dll.gr_adapters_scripts_capabilities_json.restype = ctypes.c_char_p
    dll.gr_adapters_scripts_unavailable_default_reason.argtypes = []
    dll.gr_adapters_scripts_unavailable_default_reason.restype = ctypes.c_char_p
    dll.gr_adapters_scripts_unavailable_issue_json.argtypes = [ctypes.c_char_p, ctypes.c_char_p, ctypes.c_char_p]
    dll.gr_adapters_scripts_unavailable_issue_json.restype = ctypes.c_char_p
    dll.gr_adapters_scripts_unavailable_compile_result_json.argtypes = [ctypes.c_char_p, ctypes.c_char_p, ctypes.c_char_p]
    dll.gr_adapters_scripts_unavailable_compile_result_json.restype = ctypes.c_char_p
    return dll
...
def test_native_unavailable_script_compiler_default_reason_matches_python() -> None:
    ...
    assert dll.gr_adapters_scripts_unavailable_default_reason().decode("utf-8") == compiler.reason
...
def test_native_unavailable_script_compiler_issue_matches_python_result() -> None:
    ...
    native_issue = json.loads(
        dll.gr_adapters_scripts_unavailable_issue_json(
            source.encode("utf-8"),
            game.encode("utf-8"),
            None,
        ).decode("utf-8")
...
def test_native_unavailable_script_compiler_result_matches_python_contract() -> None:
    ...
    native_result = json.loads(
        dll.gr_adapters_scripts_unavailable_compile_result_json(
            source.encode("utf-8"),
            game.encode("utf-8"),
            reason.encode("utf-8"),
        ).decode("utf-8")
...
def test_native_adapters_scripts_capabilities_document_contract_scope() -> None:
    dll = _load_dll()
    capabilities = json.loads(dll.gr_adapters_scripts_capabilities_json().decode("utf-8"))
