...
PROJECT_DIR = ROOT / "native" / "GhostRigger.Game"
DLL_PATH = ROOT / "build" / "vs" / "x64" / "Release" / "GhostRigger.Game.dll"
...
def test_game_project_declares_resource_type_files_and_exports() -> None:
    project = (PROJECT_DIR / "GhostRigger.Game.vcxproj").read_text(encoding="utf-8")
    filters = (PROJECT_DIR / "GhostRigger.Game.vcxproj.filters").read_text(encoding="utf-8")
    ...
    assert "namespace ghostrigger::game::core::game::resource_types" in public_header
    assert "namespace ghostrigger::game::core::game::resource_types" in implementation
