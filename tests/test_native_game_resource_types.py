...
PROJECT_DIR = ROOT / "native" / "GhostRigger.Core.Game"
DLL_PATH = ROOT / "build" / "vs" / "x64" / "Release" / "GhostRigger.Core.Game.dll"
...
def test_game_project_declares_resource_type_files_and_exports() -> None:
    project = (PROJECT_DIR / "GhostRigger.Core.Game.vcxproj").read_text(encoding="utf-8")
    filters = (PROJECT_DIR / "GhostRigger.Core.Game.vcxproj.filters").read_text(encoding="utf-8")
    ...
    assert "namespace ghostrigger::core::game::core::game::resource_types" in public_header
    assert "namespace ghostrigger::core::game::core::game::resource_types" in implementation
