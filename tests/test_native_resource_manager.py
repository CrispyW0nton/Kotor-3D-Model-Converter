...
PROJECT_DIR = ROOT / "native" / "GhostRigger.Assets"
DLL_PATH = ROOT / "build" / "vs" / "x64" / "Release" / "GhostRigger.Assets.dll"
...
def test_assets_project_declares_resource_manager_files_and_exports() -> None:
    project = (PROJECT_DIR / "GhostRigger.Assets.vcxproj").read_text(encoding="utf-8")
    filters = (PROJECT_DIR / "GhostRigger.Assets.vcxproj.filters").read_text(encoding="utf-8")
    ...
    assert "namespace ghostrigger::assets::core::assets::resource_manager" in public_header
    assert "namespace ghostrigger::assets::core::assets::resource_manager" in implementation
