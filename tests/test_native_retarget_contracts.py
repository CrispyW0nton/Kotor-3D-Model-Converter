...
PROJECT_DIR = ROOT / "native" / "GhostRigger.Core.Workflow"
...
def test_retargeting_project_declares_contract_files_and_exports() -> None:
    project = (PROJECT_DIR / "GhostRigger.Core.Workflow.vcxproj").read_text(encoding="utf-8")
    filters = (PROJECT_DIR / "GhostRigger.Core.Workflow.vcxproj.filters").read_text(encoding="utf-8")
    ...
    assert "namespace ghostrigger::core::retargeting::core::retargeting::retarget_contracts" in public_header
    assert "namespace ghostrigger::core::retargeting::core::retargeting::retarget_contracts" in implementation
