...
PROJECT_DIR = ROOT / "native" / "GhostRigger.Retargeting"
DLL_PATH = ROOT / "build" / "vs" / "x64" / "Release" / "GhostRigger.Retargeting.dll"
...
def test_retargeting_project_declares_contract_files_and_exports() -> None:
    project = (PROJECT_DIR / "GhostRigger.Retargeting.vcxproj").read_text(encoding="utf-8")
    filters = (PROJECT_DIR / "GhostRigger.Retargeting.vcxproj.filters").read_text(encoding="utf-8")
    ...
    assert "namespace ghostrigger::retargeting::core::retargeting::retarget_contracts" in public_header
    assert "namespace ghostrigger::retargeting::core::retargeting::retarget_contracts" in implementation
