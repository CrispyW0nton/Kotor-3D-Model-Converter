...
PROJECT_DIR = ROOT / "native" / "GhostRigger.IO"
DLL_PATH = ROOT / "build" / "vs" / "x64" / "Release" / "GhostRigger.IO.dll"
...
def test_io_declares_native_fbx_sdk_settings_files() -> None:
    project = (PROJECT_DIR / "GhostRigger.IO.vcxproj").read_text(encoding="utf-8")
    filters = (PROJECT_DIR / "GhostRigger.IO.vcxproj.filters").read_text(encoding="utf-8")
    ...
    assert "namespace ghostrigger::io::fbx::sdk_settings" in header
    assert "namespace ghostrigger::io::fbx::sdk_settings" in implementation
