...
class MDLBinaryWriter:
    ...
    def _build(self, model, is_k2: bool) -> Tuple[bytes, bytes]:
        ...
        # Child ptr arrays: one per node, after face data
        child_ptr_section_start_rel = face_section_end_rel
    ...
    def _serialize_controllers(self, node) -> Tuple[List[bytes], List[float]]:
        ...
        # No raw controllers: generate minimal bind-pose controllers.
        # We emit synthetic single-keyframe (t=0) controllers for the
        # most important node properties so parsers can read them back.
        def _emit(ctype: int, cols: int, vals: List[float]):
            nonlocal floats, entries
            t_off = len(floats)
            floats.append(0.0)   # time = 0 (bind pose)
            d_off = len(floats)
            floats.extend(vals)
            entry = bytearray(16)
            struct.pack_into('<I', entry, 0, ctype)
            struct.pack_into('<H', entry, 6, 1)      # row_count = 1
            struct.pack_into('<H', entry, 8, t_off)  # timekey_off
            struct.pack_into('<H', entry, 10, d_off) # data_off
            entry[12] = cols & 0xFF
            entries.append(bytes(entry))

        # Position controller (type=8, 3 floats)
        pos = node.position or (0.0, 0.0, 0.0)
        _emit(8, 3, [float(pos[0]), float(pos[1]), float(pos[2])])

        # Orientation controller (type=20, 4 floats: x,y,z,w)
        rot = node.rotation or (0.0, 0.0, 0.0, 1.0)
        x, y, z, w = rot
        _emit(20, 4, [float(x), float(y), float(z), float(w)])

        # Alpha controller (type=132, 1 float) — only for mesh nodes
        if (node.flags or 0) & 0x0020:
            alpha = getattr(node, 'alpha', 1.0)
            if alpha is not None and abs(float(alpha) - 1.0) > 1e-6:
                _emit(132, 1, [float(alpha)])

            # Selfillum controller (type=100, 3 floats) — only if non-zero
            si = getattr(node, 'selfillum', None)
            if si and (si[0] > 1e-6 or si[1] > 1e-6 or si[2] > 1e-6):
                _emit(100, 3, [float(si[0]), float(si[1]), float(si[2])])
