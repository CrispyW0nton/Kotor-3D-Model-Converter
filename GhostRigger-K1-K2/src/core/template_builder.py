...
def validate_animations_via_pykotor(
    ...
    # Fallback: parse using internal MDLBinaryParser
    try:
        ...
        try:
            from core.mdl_parser import MDLBinaryParser  # type: ignore
        except ImportError:
            from src.core.mdl_parser import MDLBinaryParser  # type: ignore

        parser = MDLBinaryParser(mdl_bytes, mdx_bytes)
        model  = parser.parse()
        if model is None:
            result['error'] = "MDLBinaryParser returned None"
            return result
