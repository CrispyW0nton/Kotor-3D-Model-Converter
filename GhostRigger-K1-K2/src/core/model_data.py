...
class KotorModel:
    ...
    def render_bounds(self):
        ...
        for n in self.mesh_nodes():
            ...
            if n.is_skin:
                skin_verts_by_node.append((n, wv))
            else:
                nonskin_verts.extend(wv)

        # For accessory models (non-standard supermodel), exclude skin proxy
        # meshes that are far from the non-skin geometry centroid.
        # This prevents body-proxy meshes from pulling camera too far back.
        all_verts = list(nonskin_verts)

        if nonskin_verts and is_accessory:
            # Compute centroid of non-skin geometry (the "anchor")
            ns_zs = [v[2] for v in nonskin_verts]
            ns_cz = sum(ns_zs) / len(ns_zs)
            max_ns_verts = max(len(n.vertices) for n, _ in skin_verts_by_node) if skin_verts_by_node else 0
            # Strategy B: if all non-skin nodes are tiny (face pieces ≤50 verts),
            # exclude skin nodes that are much larger (body proxies).
            _ns_vc_cands = [len(n.vertices) for n in self.mesh_nodes()
                             if not n.is_skin and n.vertices
                             and not any(n.name.lower().endswith(s)
                                         for s in ('_g','_g0','_dum'))]
            max_ns_nonskin_verts = max(_ns_vc_cands) if _ns_vc_cands else 1
            vc_threshold = max(max_ns_nonskin_verts * 5, 100)
            small_ns = (max_ns_nonskin_verts <= 50)

            # Include skin nodes whose centroid is within 1.5 units of anchor
            # AND (if all non-skin are tiny) whose vert count is not too large
            for node, wv in skin_verts_by_node:
                node_zs = [v[2] for v in wv]
                node_cz = sum(node_zs) / len(node_zs)
                z_ok = abs(node_cz - ns_cz) <= 1.5
                vc_ok = not small_ns or len(node.vertices) < vc_threshold
                if z_ok and vc_ok:
                    all_verts.extend(wv)
        else:
            # Non-accessory or no non-skin geometry: include all skin verts
            for node, wv in skin_verts_by_node:
                all_verts.extend(wv)

        if not all_verts:
            # Fall back: include all skin verts regardless
            for node, wv in skin_verts_by_node:
                all_verts.extend(wv)
    ...
    def load(cls, mdl_path: str, mdx_path: str = "") -> 'KotorModel':
        ...
        return MDLBinaryParser.parse_files(mdl_path, mdx_path)
